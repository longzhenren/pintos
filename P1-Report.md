...
王彦婷：阅读`devices/time.c`, `thread.h`, `thread.c`的代码，完成mission1的代码编辑，并进行测试

姜海洋：阅读Thread.c代码。完成mission2.1的代码编辑并进行测试
...
**王彦婷**：在实验过程中阅读代码的时候发现，在函数中，会反复验证条件是不是符合，并且保证原子操作。在实验过程中加深了对于操作系统知识的理解，尤其是关于进程忙等待，阻塞等方面的知识。通过阅读代码，更加规范了写代码的风格。在实验过程中查找了很多资料，对于一个问题其实有很多不同的解决方法，不仅是单一的和模式化的方法。在实现的过程中，需要考虑屏蔽中断等问题，比其他的编程情景更加复杂。
